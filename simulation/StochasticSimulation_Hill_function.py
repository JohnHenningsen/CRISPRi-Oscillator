...
# where a dCas9 - sgRNA pair replaces TetR (with sgRNA transcribed from
# high copy P_LTetO1 sponge).
...
l_plasmid = l_dil
K = np.array([7, 5.9, 2.3]) # Michaelis-Menten constants of repression in nM from Niederholtmeyer et al. eLife [dCas9, cI, TetR]
n = np.array([2.0, 1.9, 1,2])  # Hill coefficients from Niederholtmeyer et al. eLife (not dCas9) [dCas9, cI, TetR]
...
p = np.array([0.0, p0, 0.0])   # Define initial repressor numbers
#                                  p[0]=dCas9
#                                  p[1]=cI
#                                  p[2]=TetR
...
dCas9_sgRNA1 = 0.0 # Number of mature dCas9-dummy sgRNA complexes

# This vector is updated after each reaction of the gillespie simulation
p_free = np.array([0.0, 0.0, 0.0])  # Number of unbound repressors.
...
num_bind_sites = np.array([1.0, 2.0, 2.0])

N_repressilator_mean = 5  # Mean repressilator plasmid copy number

N_sponge_mean = 20  # Mean sponge plasmids copy numbers
...
###############################################################################
time_window_of_simulation = 50*60*60  #  in seconds
...
p_1_sim = np.array([])  # array storing simulation protein dCas9 time series
p_2_sim = np.array([])  # array storing simulation protein cI time series
p_3_sim = np.array([])  # array storing simulation protein TetR time series

dCas9_sgRNA_sim = np.array([]) # array storing simulation dCas9-sgRNA complex time series
sgRNA_sim = np.array([])  # array storing simulation sgRNA time series
...
N_rep_sim = np.append(N_rep_sim, N_repressilator)
N_spon_sim = np.append(N_spon_sim, N_sponge)
###############################################################################
...
def calculate_free_repressors():
    N = np.array([N_repressilator + 3 * N_sponge,
...
###############################################################################
# data export
t_min = t_sim[::100]  # new time vector, 1 datapoint per minute
dCas9_sgRNA_sim_save = dCas9_sgRNA_sim[::100]
p_2_sim_save = p_2_sim[::100]
p_3_sim_save = p_3_sim[::100]
GFPmat_sim_save = GFPmat_sim[::100]


# create dataframe with desired vectors
df = pd.DataFrame({'Time': t_min,
                    'cI': p_2_sim_save,
                    'dCas9-sgRNA': dCas9_sgRNA_sim_save,
                    'TetR': p_3_sim_save,
                    'GFP': GFPmat_sim_save})

filename = 'sim_run_hill_function.csv'
...
# Plot simulation
plt.figure(1)
#plt.plot(t_sim / 3600, p_1_sim, label='dCas9', linewidth=10)
...
maxim = max([np.amax(dCas9_sgRNA_sim), np.amax(p_2_sim), np.amax(p_3_sim)])
...
plt.plot(t_sim / 3600, sgRNA1_sim, label='sgRNA', linewidth=5)
...
maxim = max([np.amax(dCas9_sgRNA_sim), np.amax(dCas9_sgRNA1_sim)])
plt.axis([0, T / 3600, 0, maxim + 0.1 * maxim])
plt.tick_params(labelsize=20)
plt.xlabel('t[hours]', fontsize=20)
plt.ylabel('Number of proteins', fontsize=20)
...
plt.figure(5)
#plt.plot(t_sim / 3600, p_2_sim, label='cI', linewidth=5)
plt.plot(t_min / 60, GFPmat_sim_save, label='GFPmat', linewidth=5)
plt.tick_params(labelsize=20)
